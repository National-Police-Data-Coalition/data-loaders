...
def add_indexes():
    """
    Add indexes to the database for faster lookups.
    """
    db.cypher_query("CREATE FULLTEXT INDEX officerNames FOR (n:Officer) ON EACH [n.first_name, n.last_name, n.middle_name, n.suffix]")
    db.cypher_query("CREATE FULLTEXT INDEX allegationTypes FOR (a:Allegation) ON EACH [a.type]")
    db.cypher_query("CREATE FULLTEXT INDEX complaintCategories FOR (c:Complaint) ON EACH [c.category]")
    db.cypher_query("CREATE FULLTEXT INDEX officerRanks FOR ()-[r:MEMBER_OF_UNIT]->() ON EACH [r.highest_rank]")
    db.cypher_query("CREATE FULLTEXT INDEX cityNames FOR (c:CityNode) ON EACH [c.name]")
    db.cypher_query("CREATE FULLTEXT INDEX countyNames FOR (c:CountyNode) ON EACH [c.name]")
